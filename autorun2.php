...
$output = shell_exec("bzcat denmark-latest.osm.bz2 | osmosis  --read-xml enableDateParsing=no file=-  --bounding-box top=$top left=$left bottom=$bottom right=$right --write-xml file=- | bzip2 > $map_name.osm.bz2 2>&1");
//if ($output == NULL){
//    echo "<p>NULL<p>";
//}
//else{
...
function create_mscript_file($map_name){
	$myfile = fopen("osm_render_file.mscript", "w") or die("Unable to open file!");
        ...
        fwrite($myfile, $txt);
        $txt = "load-source ../$map_name.osm.bz2"."\n";
        fwrite($myfile, $txt);
